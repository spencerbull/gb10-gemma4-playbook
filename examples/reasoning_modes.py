...
def main() -> int:
    parser = argparse.ArgumentParser(description="Demo Gemma4 reasoning mode toggles")
    ...
    parser.add_argument(
        ...
        choices=["auto", "on", "off"],
        default="auto",
        help="Reasoning mode for this request",
    )
    args = parser.parse_args()

    thinking = {"auto": None, "on": True, "off": False}[args.mode]
    response = chat(
        args.api_base,
        args.model,
        thinking,
    ...
    print("reasoning:", json.dumps(message.get("reasoning"), ensure_ascii=False, indent=2))
