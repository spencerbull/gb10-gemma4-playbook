...
if [ "$SKIP_BUILD" -eq 0 ]; then
    ...
    remote_bash "cd '$REMOTE_REPO_DIR' && ./build-and-copy.sh --tf5"
fi

if [ "$SKIP_DOWNLOAD" -eq 0 ]; then
    log "Downloading the model on the target"
    remote_bash "cd '$REMOTE_REPO_DIR' && ./hf-download.sh '$MODEL_ID'"
fi
...
if [ "$SKIP_START" -eq 0 ]; then
    ...
    remote_bash "cd '$REMOTE_REPO_DIR' && ./run-recipe.sh '$RECIPE' --solo -d$EXTRA_ARGS"
