...
remote_bash "cd '$REMOTE_REPO_DIR' && python3 benchmarks/chat_completion_benchmark.py --api-base http://127.0.0.1:$VLLM_PORT --runs '$RUNS' --max-tokens '$MAX_TOKENS' --large-target-tokens '$LARGE_TARGET_TOKENS' --output benchmarks/gemma4-26b-a4b-it-nvfp4a16-metrics.md"

SAFE_HOST="${TARGET_HOST//[^A-Za-z0-9._-]/-}"
STAMP="$(date -u +'%Y%m%d-%H%M%S')"
LOCAL_REPORT="$REPORTS_DIR/${STAMP}-${SAFE_HOST}-gemma4-26b-a4b-it-nvfp4a16-metrics.md"

rsync -az "$SSH_TARGET:$REMOTE_REPO_DIR/benchmarks/gemma4-26b-a4b-it-nvfp4a16-metrics.md" "$LOCAL_REPORT"
